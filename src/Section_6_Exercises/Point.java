...
public class Point {
    ...
    public double distance(int x, int y) {
        int xDiff = Math.abs(Math.abs(x) - Math.abs(this.x));
        int yDiff = Math.abs(Math.abs(y) - Math.abs(this.y));
    ...
    public double distance(Point point2) {
        int xDiffPoint = point2.x - this.x;
        int yDiffPoint = point2.y - this.y;

        return Math.sqrt((Math.pow(xDiffPoint, 2)) +
                (Math.pow(yDiffPoint, 2)));
    }
